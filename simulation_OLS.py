...
import dataset_generation  # Assumiamo che accetti w_star in input
import add_noise           # Assumiamo che calcoli sigma internamente tramite SNR
import training
...
for t in range(num_trials):
    ...
    for idx, d in enumerate(d_range):
        ...
        w_hat_d =training.train_pseudoinverse(X_train_d,y_train_noisy)
        # adding the errors
...
plt.title(f'Double Descent in OLS (Trials={num_trials}, SNR={snr})', fontsize=14)
plt.legend()
